...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        self.image_paths = list(Path(self.data_root).rglob("*.jpg"))
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % args.save_every_steps == 0 and global_step > 0:
                if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        ...
        pipeline.save_pretrained(os.path.join(args.output_dir, args.logging_dir))
        # Also save the newly trained embeddings
        learned_embeds = accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[placeholder_token_id]
        learned_embeds_dict = {args.placeholder_token: learned_embeds.detach().cpu()}
        torch.save(learned_embeds_dict, os.path.join(args.output_dir, args.logging_dir, "learned_embeds.bin"))
